...
import schema from "@/cocms/service-cloud-infra";

export default async function CloudInfraPage() {
  const c = await getContent(schema);

  return (
    <main className="flex-1 bg-white">
      <section className="relative overflow-hidden bg-linear-to-br from-cyan-900 via-sky-900 to-slate-900 py-24 sm:py-32">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,var(--tw-gradient-stops))] from-cyan-500/20 via-transparent to-transparent" />
        <div className="relative mx-auto max-w-7xl px-6 text-center lg:px-8">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-lg text-cyan-100">{c.subtitle as string}</p>
        </div>
      </section>
